...
const RadarChart = ({
  ...
  data,
    ...
    const dataDimensions = [...new Set(data.yData.map(series => series.dimension))];
    ...
    const chartData = data.xData.map(date => ({
      ...
      value: sortedDataDimensions.map((dimension) => {
        const seriesForDimension = data.yData.find(series => series.dimension === dimension);
        if (seriesForDimension) {
          const index = data.xData.indexOf(date);
          return seriesForDimension.data[index] === null ? 0 : seriesForDimension.data[index]; // handle null as 0 (or null)
        } else {
