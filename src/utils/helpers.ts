...
import { globalDimension, otherDimension } from "@utils/constants";


export const isScoreSeries = (series: Visualization.DataSeries[], seriesName: string) => {
  return series.some((s) => s.name === seriesName);
}

export const getLabelFromOriginalValueAndDataSeriesName = (yData: Visualization.DataSeries[], originalValue: Domains.NumberOrNull, seriesName: string) => {
  const correspondingSeries = yData.find((series) => series.name === seriesName);
  console.log("correspondingSeries: ", correspondingSeries, originalValue);
  return correspondingSeries ? correspondingSeries.dataLabels[correspondingSeries.originalData.indexOf(originalValue)] : "";
}

// ok 
export const getOriginalValueFromNormalizedValueAndDataSeriesName = (yData: Visualization.DataSeries[], normalizedValue: number, seriesName: string) => {
  ...
  const correspondingOriginalSeries = originalData.find(
      ...
      if (correspondingOriginalSeries !== undefined && correspondingNormalizedSeries !== undefined && normalizedDataWithoutNulls !== undefined && originalDataWithoutNulls !== undefined) { // 
        correspondingNormalizedSeries.data = normalizedDataWithoutNulls;
        correspondingOriginalSeries.data = originalDataWithoutNulls;
        const dataIndex = correspondingNormalizedSeries.data.indexOf(normalizedValue);
...
// ok
export const isQuestionnaireScoreItem = (item: PromData.Item): item is PromData.QuestionnaireScoreItem => {
...
// ok
export const getMinAndMaxAnswerOptionValueForItem = (item: PromData.QuestionnaireItem) => {
...
// ok
export const createCommonTimeAxis = (questionnaireResponses: Record<string, PromData.QuestionnaireResponse>) => {
...
// ok
export const addNullQuestionnaireResponsesForCommonTimeAxisAndSortByDate = (questionnaireResponses: _.Dictionary<PromData.QuestionnaireResponse[]>, commonTimeAxisDates: Domains.DateFormat[]) => {
...
// ok
export const getUniqueQuestionnaires = (proms: Record<string, PromData.QuestionnaireResponse>) => {
...
// ok
export const calculatePeriodOfObservations =  (proms: Record<string, PromData.QuestionnaireResponse>) => {
  ...
  if(uniqueYears.length < 2) {
    ...
    return [uniqueYears[0].toString(), uniqueYears[uniqueYears.length - 1].toString()];
  }
}

export const createDateQuestionnairesRecord = (proms: Record<string, PromData.QuestionnaireResponse>) => {
  ...
  Object.keys(questionnairesByDate).sort((a, b) => new Date(a).getTime() - new Date(b).getTime()).forEach((key) => {
...
export const createQuestionnaireChartDataRecord = (questionnaires: PromData.Questionnaire[], chartData: Visualization.ChartData) => {
  const chartDataByQuestionnaire: Record<string, Visualization.ChartData> = {};
  questionnaires.forEach((questionnaire) => {
      const questionnaireData = chartData.yData.filter(data => data.questionnaire === questionnaire.id);
    ...
    const chartDataByQuestionnaireWithoutNulls: Record<string, Visualization.ChartData> = {};
    Object.keys(chartDataByQuestionnaire).forEach((questionnaireId) => {
      ...
      if (columnsToRemove.length > 0) {
        ...
        const newYData = yData.map(series => {
          return {...series, data: series.data.filter((_, index) => !columnsToRemove.includes(chartData.xData[index]))};
        });
...
export const createDimensionChartDataRecord = (dimensions: string[], questionnaires: PromData.Questionnaire[], scoresDataSeries: Visualization.DataSeries[], itemsDataSeries: Visualization.DataSeries[], xData: string[]) => {
  const chartDataByDimension: Record<string, Visualization.ChartData> = {};
  dimensions.forEach((dimension) => {
    const scores = scoresDataSeries.filter(score => score.dimension === dimension);
    const items = itemsDataSeries.filter(item => item.dimension === dimension);
    // Filter
    scores.forEach((score) => {
      const scoreQuestionnaire = questionnaires.find(q => Object.keys(q.items).includes(score.id));
      if (scoreQuestionnaire) { 
        const scoreItem = scoreQuestionnaire.items[score.id] as PromData.QuestionnaireScoreItem;
        const itemsToBeRemoved = scoreItem.referenceQuestionnaireItems?.map((linkId) => items.find(item => item.id === linkId)).filter(item => item !== undefined);
        if (itemsToBeRemoved) {
