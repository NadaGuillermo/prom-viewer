...
export const normalizeObservationDefinition = (
  ...
  const observationRange = resource.qualifiedInterval?.find(
    (interval) =>
      interval.category !== undefined && interval.category === "absolute",
  )?.range;
  ...
  const range =
    lowerBoundary !== undefined && upperBoundary !== undefined
      ? ([lowerBoundary, upperBoundary] as [number, number])
      : undefined;
  const scoreHealthCorrelation: string | undefined =
    observationRange?.extension?.find(
      (ext) =>
        ext.url ===
  ...
  const referenceIntervals = resource.qualifiedInterval?.filter(
    (interval) =>
      interval.category !== undefined &&
      (interval.category === "critical" || interval.category === "reference"),
  );
  const referenceRangeRaw:
    ...
    | undefined = referenceIntervals
    ? referenceIntervals.map((interval) => {
        return {
          range: [interval.range?.low?.value, interval.range?.high?.value] as [
            number | undefined,
            number | undefined,
          ],
          context: interval.context?.coding?.find(
            (coding) =>
              coding.system ===
  ...
  return {
    data: {
      ...
      url: resource.url!, // should alwyas be given
      range: range,
